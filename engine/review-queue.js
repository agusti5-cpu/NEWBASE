...
export function getDue(queue, now = new Date().toISOString()) {
  ...
  return queue.filter((item) => !item.exhausted && Number.isFinite(new Date(item.nextEligibleAt).getTime()) && new Date(item.nextEligibleAt).getTime() <= nowMs);
}
...
export function markRechecked(queue, opportunityId, checkedAt, outcome, options = {}) {
  ...
  return queue.flatMap((item) => {
    ...
    if (next?.status === 'accepted' || next?.status === 'publishable') return [];
    const maxAttempts = Number.isFinite(options.maxAttempts) ? Math.max(1, options.maxAttempts) : (item.maxAttempts ?? DEFAULT_MAX_ATTEMPTS);
    const attempts = item.attempts + 1;
    const exhausted = attempts >= maxAttempts;
    return [{ ...item, reason: next?.reason ?? item.reason, attempts, maxAttempts, exhausted, lastCheckedAt: checkedAt, nextEligibleAt: exhausted ? null : addHours(checkedAt, delayHours) }];
  });
}

export function isRetryableRejection(result) { return retryable(result); }
